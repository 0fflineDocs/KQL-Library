...
const Input = ({ className = "", ...props }: any) => (
  <input
    className={cn("w-full px-3 py-2 rounded bg-blue-900 border border-blue-800 text-white placeholder:text-blue-300", className)}
    {...props}
  />
);
...
const KQLLibrary = () => {
  ...
  return (
    <div className="min-h-screen bg-blue-950 text-gray-100 flex flex-col">
      <header className="bg-blue-950/90 backdrop-blur-md sticky top-0 z-40 border-b border-blue-900">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-4">
              ...
              className="w-64 bg-blue-900 border-blue-800 text-white placeholder:text-blue-300"
            />
            <Button
            ...
            <Button
              ...
              className={cn(
                ...
                selectedCategory === category
                  ? "bg-blue-800 ring-1 ring-orange-500"
                  : ""
              )}
              onClick={() => {
          ...
          <div className="flex flex-wrap justify-center gap-4 mb-6">
            {categoryInfo[selectedCategory].subCategories.map((subCategory: string) => (
              <Button
                key={subCategory}
                className={cn(
                  "px-4 py-2 rounded-md bg-blue-900 hover:bg-blue-800",
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredQueries.length === 0 ? (
            ...
            filteredQueries.map((query, index) => (
              <Card 
                key={index} 
                className="border-blue-900 hover:shadow-lg hover:shadow-blue-900/20 transition-all duration-300 bg-blue-900/90 backdrop-blur-md flex flex-col transform hover:-translate-y-1"
              >
                <CardHeader className="bg-gradient-to-r from-blue-950 to-blue-900 rounded-t-lg border-b border-blue-900">
                ...
                <CardContent className="flex-1">
                  <ScrollArea className="h-full w-full rounded-md border border-blue-900 bg-blue-950 mt-2">
                    <pre className="p-4 text-sm text-blue-100 whitespace-pre-wrap break-words">
                  ...
                  {query.tags && query.tags.length > 0 && (
                    <div className="mt-4 flex flex-wrap gap-2">
                      {query.tags.map((tag, i) => (
                        <span 
                          key={i} 
                          className="text-xs px-2 py-1 rounded-full bg-blue-900 text-blue-200"
                        >
                          {tag}
                        </span>
                      ))}
